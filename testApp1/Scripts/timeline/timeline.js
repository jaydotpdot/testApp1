/*
 * @file timeline.js
 * Alberta Education Timeline JavaScript Component v1.0
 * education.alberta.ca
 * 
 * Includes CHAP Links library Timeline
 * almende.github.io/chap-links-library/timeline.html
 *
 * Uses timeline-locales.js 
 * NOTE: I removed all lanuages except en and fr
 * almende.github.io/chap-links-library/js/timeline/timeline-locales.js
 *
 * Uses jQuery.js
 * jquery.com
 *
...
(function (global, M, S) {
    ...
    var govTimeline = function (data, language) {
        return govTimeline.init(data, language);
        //// auto return new instance of the timeline (unnecessary)
        //return new govTimeline.init(data, language);
    };

    // allow velocity to work with with plain javascript (if jQuery is removed in the future)
    var V = (global.jQuery) ? $.Velocity : Velocity;

    // browser feature checks
    var hasCssAnimations = M.cssanimations;
    var hasCssTransitions = M.csstransitions;

    // set class to body if csstransforms are enabled
    var body = document.getElementsByTagName('BODY')[0];
    if (hasCssAnimations) { body.className += " cssanimations"; }
    else { body.className += " nocssanimations"; }
    if (hasCssTransitions) { body.className += " csstransitions"; }

    // instance references
    var interface = {}; // interface timeline instance
    var scaffold = {}; // scaffold timeline instance
    var scroll; // iScroll instance

    // status variables
    var isFirstExpand = true; // false once the timeline has been expanded
    var isFirstRender = true; // false after different events are loaded in the timeline
    var isCollapsed = true;
    var isAnimating = false;
    var isTclAttached = false; // check if timeline control listeners are attached
    var isInterface;
    var isScaffold;
    ...
    var settings = {
        // width of timeline
        timelineWidth: window.screen.width > 1600 ? window.screen.width : 1600,

        // bezier curve for slide animations
        slideBezier: [0.36, 0.005, 0.035, 1.005],

        // collapsed event height in pixels
        eventMinHeight: 7,

        // maximized event height in pixels
        eventMaxHeight: 44
    };
    var eventLength; // number of events

    // object to store various properties of a selected event on the timeline
    var selectedEvent = {
        id: undefined, // event id
        domId: undefined, // dom id
        row: undefined, // index of the row in the timeline
        dom: undefined,
    ...
    /**
     * Constructor. Main initialization function. This needs to be called to 
     * create the timeline and set the default values. 
     * @param {function || string} data identifier    
     * @param {String} lang      Localization code (e.g. "en")
     */
    govTimeline.init = function (data, language) {

        var self = this;
        self.language = language || "en";

        if (typeof data === "function") {
            // function was passed in directly, so invoke it
            self.data = data();
        }
        else if (typeof data === "string") {
            // user used dropdown
            if (data == "school") {
                self.data = fetchSchoolEventData();
            }
            else if (data == "all") {
                self.data = fetchAllEventData();
            }
            else if (data == "ec") {
                self.data = fetchEcEventData();
            }
            else if (data == "el") {
                self.data = fetchElEventData();
            }
            else if (data == "jh") {
                self.data = fetchJhEventData();
            }
            else if (data == "hs") {
                self.data = fetchHsEventData();
            }
            else if (data == "mi") {
                self.data = fetchMinistryEventData();
            }
            else if (data == "sa") {
                self.data = fetchSaEventData();
            }
            else {
                throw "Unknown data value. String not recognized.";
            }
        }
            ...
            if (Object.prototype.toString.call(data) === "[object Array]") {
                ...
                throw "Unknown data type. Array expected.";
            }
        }
        else {
        ...
        // create timeline
        _createTimeline(self.data, self.language);

        // generate values to enable event animations
        _generateEventAnimationValues();

        // set the position of the timeline on load
        _setTimelineLayoutPosition(isCollapsed);

        // attach timeline events to events
        _attachTimelineEventListeners();

        // timeline scroll settings
        _timelineScrollSettings();

        //self.selectedEvent = selectedEvent;

        // attach event listeners to the timeline elements
        // TODO: dynamically create the elements these events are attached to,
        //       and then you can remove the isTclAttached flag
        if (!isTclAttached) {
            _attachTimelineControlListeners();
        }
    };
    ...
    // PUBLIC METHODS
    //
    govTimeline.prototype = {
        // any public/API methods that need to be exposed must go here
        // NOTE: "this" will refer to this from init method
        // NOTE: methods to be defined in object notation

        /**
         * Scroll to next event based on the currently focused event
         * @param {number} row    index of currently focused event
         */
        scrollNextEvent: function (row) {
            row = typeof row !== undefined ? row : selectedEvent.row;
            if (row < (eventLength - 1)) {
                scroll.scrollToElement($("#tl-interface button").eq(row + 1)[0], 600, true, 0);
            }
        },


        /**
         * Scroll to previous event based on the currently focused event
         * @param {number} row    index of currently focused event
         */
        scrollPrevEvent: function (row) {
            row = typeof row !== undefined ? row : selectedEvent.row;
            if (row > 0) {
                scroll.scrollToElement($("#tl-interface button").eq(row - 1)[0], 600, true, 0);
            }
        ...
        /**
         * Unselect the currently selected timeline event
         */
    ...
    // access public methods from the init method's prototype
    govTimeline.init.prototype = govTimeline.prototype;

    // add timeline to the global object (with the alias "tl")
    global.govTimeline = global.tl = govTimeline;
    ...
    /**
     * Create timeline
     * This timeline consists of two parts: the "Scaffold" and the "Interface".
     *
     * "Scaffold" indicates that the instance aids in the positioning and structure 
     * of the timeline interface, mainly for animation purposes.
     *
     * "Interface" indicates that the instance is visible to the user and that
     * the events on the screen can be interacted with.
     * 
     * @param {Array} data       Array of JSON data
     * @param {String} lang      Localization code (e.g. "en")
     */
    var _createTimeline = function (data, lang) {
        ...
        var createScaffold = function () {
            ...
            var options = {
                ...
                width: settings.timelineWidth + "px",
                moveable: false,
                groupsWidth: 0,
                eventMargin: 12,
                eventMarginAxis: 8,
                zoomMin: (1000 * 60 * 60 * 24 * 31) * 13, // value in milliseconds (13 is the approx. # of months)
                zoomMax: (1000 * 60 * 60 * 24 * 31) * 13, // value in milliseconds (13 is the approx. # of months)
                zoomable: false,
                selectable: false,
                //unselectable: false,
            ...
            // instantiate our timeline scaffold object.
            var myScaffoldEl = document.getElementById('tl-scaffold');
            scaffold = new links.Timeline(myScaffoldEl, options);
            ...
            // set the scale
            scaffold.setScale(this.links.Timeline.StepDate.SCALE.MONTH, 1);
            //scaffold.setAutoScale(false);

            // set date range
            scaffold.setVisibleChartRange(new Date(new Date().getFullYear(), 0, 31), new Date(new Date().getFullYear(), 11, 31));
        };
        ...
        var createInterface = function () {
            ...
            var options = {
                ...
                groupsWidth: 0,
                eventMargin: 4,
                eventMarginAxis: -12 + (-15),//23, // -3 to get pixel perfection in collapsed view, -12 to compensate for margin-top of div.timeline-event-range.custom
                height: "40px",
                moveable: false,
                zoomMin: (1000 * 60 * 60 * 24 * 31) * 13, // value in milliseconds (13 is the approx. # of months)
                zoomMax: (1000 * 60 * 60 * 24 * 31) * 13, // value in milliseconds (13 is the approx. # of months)
            ...
            // instantiate our timeline interface object.
            var myInterfaceEl = document.getElementById('tl-interface');
            interface = new links.Timeline(myInterfaceEl, options);
            ...
            // set the scale
            //interface.setScale(this.links.Timeline.StepDate.SCALE.DAY, 4);
            interface.setScale(this.links.Timeline.StepDate.SCALE.MONTH, 1);

            // set date range
            interface.setVisibleChartRange(new Date(new Date().getFullYear(), 0, 31), new Date(new Date().getFullYear(), 11, 31));
        };
        ...
        eventLength = interface.getData().length;
    };
     ...
     * Generate values to enable event animations
     */
    var _generateEventAnimationValues = function () {
        var sEl = document.getElementById('tl-scaffold');
        var scaffoldHeight = sEl.clientHeight;

        // check between Jan 1st of current year and Jan 1st of next year
        var nDate = new Date();
        var start = new Date(nDate.getFullYear(), 0, 1);
        var end = new Date((nDate.getFullYear() + 1), 0, 1);

        // get indices of visible events
        var visibleIndices = [];
        visibleIndices = interface.getVisibleItems(start, end);
        var len = visibleIndices.length;
        ...
        if (len) {
            for (var i = 0; i < len; i++) {
                var vRow = visibleIndices[i].row;
                var iData = interface.getItem(vRow);
                var domId = "iTl" + iData.id;

                iData.domId = domId;
                var iDom = document.getElementById(domId);
                iDom.setAttribute("data-row", vRow);
                var btn = iDom.querySelector(".timeline-event-content");
                btn.setAttribute("data-row", vRow);
                if (iDom !== "undefined") {
                    iData.top = parseFloat(iDom.style.top, 10);
                }
                //iItems.push(iData);

                var sData = scaffold.getItem(vRow);
                domId = "sTl" + sData.id;
                sData.domId = domId;
                var sDom = document.getElementById(domId);
                if (sDom !== "undefined") {
                    sData.top = parseFloat(sDom.style.top, 10);
                }
                //sItems.push(sData);

                // set event animation position
                var iEventTop = parseFloat(iDom.style.top, 10);
                var sEventHeight = $(sDom).outerHeight(true);
                var sEventTop = parseFloat(sDom.style.top, 10);
                //var translateTo = (iEventTop + scaffoldHeight) - ((sEventTop + sEventHeight) - 7);
                var topTo = ((scaffoldHeight * -1) + (sEventTop + sEventHeight)) - sEventHeight;
                //var topTo = sEventTop * -1;
                /* DEBUG if (sData.id == 1) {
                console.log(sDom);
                console.log("sTl1 var topTo = " + topTo);
                console.log("sTl1 scaffoldHeight = " + scaffoldHeight);
                console.log("sTl1 sEventTop = " + sEventTop);
                console.log("sTl1 sEventHeight = " + sEventHeight);
                } */
                /*iDom.setAttribute('data-translate-animate', 
                                  "-webkit-transform: translateY(" + translateTo + ");" + 
                                  "-moz-transform: translateY(" + translateTo + ");" + 
                                  "-ms-transform: translateY(" + translateTo + ");" +  
                                  "-o-transform: translateY(" + translateTo + ");" + 
                                  "transform: translateY(" + translateTo + ");");*/
                iDom.setAttribute('data-style-original', iDom.getAttribute('style'));
                iDom.setAttribute('data-top-animate', topTo);
                iDom.setAttribute('data-top-original', iEventTop + 'px');
            }
        }
    };
    ...
    /**
     * Set the position of the timeline in the layout
     * This determines whether the timeline is expanded or collapsed on page load
     * @param {Boolean} setCollapsed - true to collapse timeline, false to expand it
     */
    var _setTimelineLayoutPosition = function (setCollapsed) {
        setCollapsed = typeof setCollapsed !== 'undefined' ? setCollapsed : false;

        var cEl = document.getElementById('content');
        var h = document.getElementsByTagName('HEADER')[0];
        var tWrapEl = document.getElementById('tlWrapper');
        var scaffoldEl = document.getElementById('tl-scaffold');
        var iHeight = 40; //interfaceEl.clientHeight; // hardcoded to 50px
        //var interfaceEl = document.getElementById('tl-interface');
        var hHeight;
        var totalHeight;

        // set css values to collapse the timeline 
        setTimeout(function () {
            hHeight = h.clientHeight;
            totalHeight = scaffoldEl.clientHeight + iHeight;
            tWrapEl.style.height = totalHeight + "px";

            if (setCollapsed) {
                /* DEBUG
                console.log("scaffoldEl.clientHeight = " + scaffoldEl.clientHeight);
                console.log("hHeight = " + hHeight);
                var dif = scaffoldEl.clientHeight - hHeight;
                console.log("scaffoldEl.clientHeight - hHeight = " + dif);
                */
                // set timeline to be collapsed on load
                cEl.style.top = (-1 * (scaffoldEl.clientHeight - hHeight)) + "px";
            }

            _animateTimeline(setCollapsed);
        }, 100);
     ...
     */
    var _selectEvent = function (e) {
        e.preventDefault();

        // event validation checks
        /*if (e.type == "touchstart") {
          // if user somehow didn't select a button, exit
          if (e.currentTarget.nodeName != "BUTTON") {
            return;
          }
        }
        else */if (e.type == "keyup") {
        // if the key pressed on an event isn't the enter key, exit
        if (e.keyCode != 13) {
            if (event.shiftKey && event.keyCode == 9) { // shift-tab
                govTimeline.prototype.scrollPrevEvent($(e.target).data("row"));
            }
            else if (e.keyCode == 9) { // tab
                govTimeline.prototype.scrollNextEvent($(e.target).data("row"));
            }
            return; // don't select the element if only tabbing through
        }
    }
        /*else if (e.type == "mouseup") {
          // if user didn't use the left mouse button, exit
          var leftButtonDown = e.which ? (e.which == 1) : (e.button == 1);
          if (!leftButtonDown) {
            return;
          }
        }*/
        ...
        console.log(e);
        */
        console.log("selectEvent(), e = " + e.type);

        if (selectedEvent.row === "undefined") {
            console.log("WARNING: selectedEvent.row === undefined");
            // user is selecting their very first event
            _unhighlightNearestEvent();
        }

        // hijack/simulate component mouse events
        if (e.type == "keyup") {
            _triggerMouseEvent(e.srcElement, "mousedown");
            _triggerMouseEvent(e.srcElement, "mouseup");
        }
        /*else if (e.type == "mouseup") {
          _triggerMouseEvent(e.srcElement, "mousedown");  
        }
        else if (e.type == "touchstart") {
          // DEBUG $('header .actions').html("user touched: " + e.currentTarget.children[0].innerHTML);
          _triggerMouseEvent(e.srcElement, "mousedown"); 
          _triggerMouseEvent(e.srcElement, "mouseup"); 
        }*/
    };
    ...
    /**
     * Attach event listeners for timeline control interactions
     */
    var _attachTimelineControlListeners = function () {
        // click/touch events to minimize/collapse timeline
        var tf = document.getElementById("tl-footer");
        tf.addEventListener("click", function (e) { _toggleTimelineView(e); }, false);
        tf.addEventListener("touchstart", function (e) { _toggleTimelineView(e); }, false);

        // click/touch to collapse timeline
        var tc = document.querySelector("#tl-actions .timeline-close");
        tc.addEventListener("click", function (e) { _toggleTimelineView(e); }, false);
        tc.addEventListener("touchstart", function (e) { _toggleTimelineView(e); }, false);

        // wish I made a comment when I added this...
        // think it kills the component touchmove event to prevent interference with iScroll
    ...
    /**
     * Attach event listeners for timeline event interactions
     */
    var _attachTimelineEventListeners = function () {
        // keypress to select events
        var te = document.querySelectorAll("#tl-interface button"), // only month events are "buttons"
            len = te.length;
        if (len) {
            while (len--) {
                (function (lenNew) {
                    te[lenNew].addEventListener("keyup", function (e) { _selectEvent(e); }, false);
                }(len));
            }
        }

        // hover effect for "today" line
        $(".timeline-currenttime").each(function () {
            $(this).hover(function () {
                $("#tl-interface .timeline-currenttime").toggleClass("show-date");
            });
        });
    };
    ...
    /**
     * Trigger the animation of the timeline and its events
     * This is needed to enable the expand/collapse animation
     * @param {Boolean} isCollapsed     true to expand, false to collapse
     */
    var _animateTimeline = function (isCollapsed) {
        //console.log("isCollapsed: " + isCollapsed);
        //var body = document.getElementsByTagName('BODY')[0];
        //var tv = document.querySelector("#tl-footer .toggle-view");
        var tvi = document.querySelector("#tl-footer .toggle-view i");
        var delay = (isCollapsed || $(body).hasClass('touchstart')) ? 100 : 0;

        if (isCollapsed) {
            $(body).removeClass('timeline-expanded');

            tvi.setAttribute("style", "");
            tvi.className = "fa fa-angle-down";

            // switch title attribute "Minimize" to "Expand" 
            //tvi.setAttribute("title", "Expand Timeline");
        }
        else {
            $(body).addClass('timeline-expanded');

            tvi.setAttribute("style", "");
            tvi.className = "fa fa-angle-up";

            // switch title attribute "Expand" to "Minimize" 
            //tvi.setAttribute("title", "Minimize Timeline");
        }

        //
        // Animate the container/wrapper
        //
        var cEl = document.getElementById('content');
        var scaffoldEl = document.getElementById('tl-scaffold');
        var h = document.getElementsByTagName('HEADER')[0];
        ...
        if (isCollapsed) {
            V(cEl, {
                top: "-" + (scaffoldEl.clientHeight - hHeight) + "px"
            }, {
                duration: 600,
                easing: settings.slideBezier
            });
        }
        else {
            V(cEl, {
        ...
        //
        setTimeout(function () {
            var sEvents = document.querySelectorAll('#tl-interface .timeline-event-range'),
                len = sEvents.length;

            if (len) {
                // trigger animation
                while (len--) {
                    (function outer(lenNew) {
                        var el = sEvents[lenNew];
                        var isMonthEvent = $(el).hasClass("month-item");
                        var btn = el.querySelector(".timeline-event-content");
                        if (isCollapsed) {
                            // collapse event
                            V(el, {
                                top: parseFloat($(el).data('top-original'), 10)// + 5
                            }, {
                                duration: 0
                            });
                            V(el, {
                                height: isMonthEvent ? "40px" : settings.eventMinHeight + "px",
                                opacity: !isMonthEvent ? 0 : 1 /* Hide events in collapsed view */
                            }, {
                                duration: (hasCssAnimations) ? 0 : 600,
                                easing: settings.slideBezier
                            });

                            // fade out event content
                            //el = el.querySelector(".timeline-event-content");
                            V(btn, {
                                opacity: isMonthEvent ? 1 : 0,
                            }, {
                                duration: (hasCssAnimations) ? 0 : 50,
                                easing: settings.slideBezier
                            });
                        }
                        else {
                            // expand event
                            V(el, {
                                height: isMonthEvent ? "40px" : settings.eventMaxHeight + "px",
                                top: $(el).data('top-animate') + "px",
                                opacity: !isMonthEvent ? 1 : 1 /* Hide events in collapsed view */
                            }, {
                                duration: (hasCssAnimations) ? 0 : 1000,
                                easing: settings.slideBezier
                            });

                            // fade in event content
                            //el = el.querySelector(".timeline-event-content");
                            V(btn, {
                                opacity: 1,
                            }, {
                        ...
                        // add event hover animation for older browsers
                        if (!hasCssAnimations) {
                            $(btn).hover(
                              function () {
                                  var ct = this.querySelector(".content-title");
                                  var cd = this.querySelector(".content-date");
                                  var isMonthEvent = $(this).parent().hasClass("month-item");
                                  // slide up content
                                  V(ct, {
                                      marginTop: isMonthEvent ? "0px" : "4px"
                                  }, {
                                      duration: 280,
                                      easing: settings.slideBezier
                                  });
    ...
    /**
     * Toggle between expanded and collapsed timeline views
     * @param {Event} e     The event that occured
     */
    var _toggleTimelineView = function (e) {
        e.preventDefault();

        var elAnimate = document.querySelector("#tl-footer .toggle-view i");
        elAnimate.setAttribute("style", "");

        if (e !== "undefined") {
            var body = document.getElementsByTagName('BODY')[0];
            if (e.type == "touchstart" && !$(body).hasClass('touchstart')) {
                body.className += " touchstart";
            }
        }

        isAnimating = true;

        if (isCollapsed) { // setup for expanded view         
            isCollapsed = false;

            // slide down the down arrow icon into view
            elAnimate.className = "fa fa-angle-down";

            _animateTimeline(isCollapsed);

            if (isFirstExpand) { isFirstExpand = false; }
        }
        else if (!isCollapsed) { // setup for collapsed view   
            isCollapsed = true;

            // slide down the up arrow icon out of view
            elAnimate.className = "fa fa-angle-up";
    ...
    /**
     * Set properties of a selected event
     * @param {index}      row index of the event in the timeline
     * @return {Object}    An object with properties of the selected event
     */
    var _setSelectedEvent = function (index) {
        var sel = {};
        // retrive data for the selected event 
        if (interface !== "undefined") {
            sel = interface.getSelection();
            if (sel.length) {
                if (sel[0].row !== "undefined") {
                    selectedEvent.row = sel[0].row;
                    selectedEvent.data = interface.getItem(selectedEvent.row);
                }
                // get the element from the dom
                sel = document.querySelector('#tl-interface .timeline-event-selected');
            }
            else {
                // element wasn't selected by user, it's just styled to be highlighted
                // this is because we "select" the nearest event on page load
                console.log("WARNING: The event wasn't selected, only highlighted");
                // get the element from the dom
                sel = document.querySelector('#tl-interface .highlight-nearest-event');

                // set the title (because we can)
                selectedEvent.data.content = $(sel).find(".content-title").html();
            }
        }

        // set id's
        selectedEvent.domId = sel.id;
        selectedEvent.id = sel.id.replace("iTl", "");

        // cache element
        selectedEvent.dom = sel;

        // store important dimensions
        selectedEvent.rangeWidth = parseFloat(selectedEvent.dom.getAttribute('data-range-width'));
        selectedEvent.titleWidth = parseFloat(selectedEvent.dom.getAttribute('data-title-width'));
        selectedEvent.renderedWidth = parseFloat(selectedEvent.dom.style.width);

        $('.cardWrapper h4').html(
          "<p style='padding: 0 10px'>selectedEvent.data.content: " + selectedEvent.data.content + "</p>");

        // 1.
        if (scroll != undefined) {
            setTimeout(function () {
                scroll.scrollToElement(selectedEvent.dom, 600, true, 0);
            }, 0);
        }
    ...
    /**
     * Timeline scroll settings
     * 1) Sets the default values for iScroll plugin - DONE
     * 2) Sets custom logic to determine scroll position on load to try and always display
     *    the current/upcoming event first in the timeline
     * 3) Enables animation to center selected event
     */
    var _timelineScrollSettings = function () {
        setTimeout(function () {
            // initiate smooth horizontal scrolling
            scroll = new S('#tl-scroll', {
                scrollX: true,
                scrollY: false,
                useTransform: false, // removing this breaks "scrollToElement"
                //eventPassthrough: true, // preserve native vertical scroll, add horizontal iScroll
                mouseWheel: true,
                //momentum: false,
                deceleration: 0.002,
                startX: 0,
                startY: 0,
                snap: '.timeline-event.month-item'//,
                //tap: true
            });

            // call iScroll refresh method
            //scroll.refresh();

            // scroll to the inital selected event
            scroll.scrollToElement(selectedEvent.dom, 600, true, 0);
        }, 100);
    };
     ...
     * Bug: cannot set max width for an item, like .timeline-event-content {white-space: normal; max-width: 100px;}
     * Bug on IE in Quirks mode. When you have groups, and delete an item, the groups become invisible
     */

    /*
     * JP Change log
     * - Commented out section that renders timeline when it's moved 
     *   (find comment "timeline is moved"). Since we are now moving the timeline
     *   with the iScroll plugin, items don't need to be re-rendered in this way.
     * - Removed the method "mapColumnIds", as it was specific to google API
     * - Removed ItemBox and ItemDot methods
     * - Removed all references to google API
     * - Removed methods for onMouseWheel and onDblClick
     * - Removed "getClusterIndex" method
     */
     ...
     * JP Updates
     * - Commented out references to the itemTypes of ItemBox and ItemDot
     *
     ...
     * JP Updates
     * 1. - Changed code to set visible chart range based on specified state date.
     *
    ...
    links.Timeline.prototype.draw = function (data, options) {
        ...
        if (this.firstDraw) {
            ...
            this.setVisibleChartRange(this.options.start); // 1.
        }
    ...
    links.Timeline.prototype.setOptions = function (options) {
        ...
        if (options) {
            ...
            if (typeof links.locales !== 'undefined' && options.locale !== 'en') {
                var localeOpts = links.locales[options.locale];
                if (localeOpts) {
     ...
     * 1. - Added "upcomingItemIndex" variable store the index of the item closest to today so it can be 
     *      selected on page load
     * 2. - Added logic to remove timestamps from state/end dates so things line up nicely on the timeline
     * 3. - Addded loop to create unique IDs for events (in the interface and the scaffold)
     * 4. - Update the code to only auto select "month" events
     * @param {Array} data
     */
    links.Timeline.prototype.setData = function (data) {
        ...
        if (links.Timeline.isArray(data)) {
            ...
            for (var row = 0, rows = data.length; row < rows; row++) {
                ...
                // 2. - remove timestamps from state/end dates
                if (itemData.start !== "undefined" && Object.prototype.toString.call(itemData.start) === "[object Date]") {
                    itemData.start = itemData.start.setHours(0, 0, 0, 0);
                }
                if (itemData.end !== "undefined" && Object.prototype.toString.call(itemData.end) === "[object Date]") {
                    itemData.end = itemData.end.setHours(23, 0, 0, 0);
                }
                var item = this.createItem(itemData);
                items.push(item);

                // capture index of event that's either current or closest to today
                if (upcomingItemIndex === undefined) {
                    // first check if it's a range (because ranges can start before today), 
                    // and see if today falls between the range
                    if (item.className == "month-item") { // 4.
                        if (itemData.end && (now.getTime() > itemData.start && now.getTime() < itemData.end)) {
                            upcomingItemIndex = row;
                            // set title to selected event?
                            document.title = "Alberta Education |  Event in progress: " + items[upcomingItemIndex].content;
                        }
                        else if (itemData.start >= now.setHours(0, 0, 0, 0)) {
                            // check if the one-day-event is coming up next
                            upcomingItemIndex = row;
                            // JP - set title to selected event?
                            document.title = "Alberta Education | Upcoming Event: " + items[upcomingItemIndex].content;
                        }
                    }
                }
            }
        ...
        // 3. - set unique IDs to the items
        for (var i = 0; i < data.length; i++) {
            var event = items[i];
            var dom = event.dom;
            if (isInterface) {
                dom.id = "iTl" + data[i].id;
            }
            else if (isScaffold) {
                dom.id = "sTl" + data[i].id;
            }
        }
        /* DEBUG
        console.log("IMPORTANT: Remove hardcoded upcomingItemIndex from Prod!");
        upcomingItemIndex = 7;
        */
        // JP - manually set classes to the current/upcoming event so it appears selected
        var upItem = items[upcomingItemIndex];
        var upDom = upItem.dom;
        if (upDom != undefined) {
            //links.Timeline.addClassName(upDom, 'highlight-nearest-event');

            this.selectItem(upcomingItemIndex);
            if (isInterface) {
                _setSelectedEvent(upcomingItemIndex);
            }
        }
        else {
            console.log('Could not indentify the nearest event');
        }
    };
    ...
    links.Timeline.prototype.render = function (options) {
        ...
        //var resized = (frameResized || axisResized || groupsResized || itemsResized); // 1.
     ...
     * JP Updates
     * 1. - adjust top to show months above the year
     * @param {Number} x
     * @param {String} text
     */
    links.Timeline.prototype.repaintAxisMinorText = function (x, text) {
        ...
        var adjust = isInterface ? 8 : 0; // 1.
        label.style.top = (size.axis.labelMinorTop - adjust) + "px";
     ...
     * JP Updates
     * 1. - Modify items to use the width of the child "title" element for improved stacking
    ...
    links.Timeline.prototype.reflowItems = function () {
        ...
        for (i = 0, iMax = renderedItems.length; i < iMax; i++) {
            ...
            if (domItem) {
                ...
                // 1.
                var contentWidth = domItem.children[0].childNodes[0].offsetWidth;
                var dateWidth = domItem.children[0].childNodes[1].offsetWidth;
                var titleWidth = Math.max(contentWidth, dateWidth);
     ...
     * Repaint all items
     * JP Updates
     * 1. - Added "options" parameter to showDOM method
    ...
    links.Timeline.prototype.repaintItems = function () {
        ...
        while (item = queue.show.shift()) {
            item.showDOM(frame, this.options); // 1.
        ...
        renderedItems.forEach(function (item) {
            ...
            /* DEBUG
            if (interfaceComplete && item.type == "range") {
            //debugger;    
              } */
     ...
     * JP Updates
     * 1. - removed timestamp from today's date to line up the line correctly
     */
    links.Timeline.prototype.repaintCurrentTime = function () {
        ...
        var x = this.timeToScreen(now.setHours(0, 0, 0, 0)); // 1.
        ...
        var todayFormat = options.MONTHS_SHORT[now.getMonth()] + " " + now.getDate() + ", " + now.getFullYear();
        dom.currentTime.setAttribute("data-current-date", todayFormat);
     ...
     * JP Updates
     * 1. - Added check to prevent user from clicking events while animation is running
     * 2. - Call "setSelectedEvent" to populate data once an item is selected
     * 3. - Only allow "month" events to be selected
     * This function activated from within the funcion links.Timeline.onMouseDown().
                ...
                if (params.target === this.dom.items.deleteButton) {
                    ...
                    if (params.itemIndex != undefined) {
                        ...
                        if (!this.isSelected(params.itemIndex)) {
                            if (isAnimating === undefined || !isAnimating) { // 1.
                                var item = this.items[params.itemIndex];
                                if (item.className == "month-item") { // 3.
                                    this.selectItem(params.itemIndex);
                                    this.trigger('select');
                                    // 2.
                                    _setSelectedEvent(params.itemIndex);
                                }
                                //else if (!isCollapsed) {
                                //  this.selectItem(params.itemIndex);
                                //  this.trigger('select');                              
                                ...
                                console.log("WARNING: Cannot select new event while animating");
                            }
                        }
                        //}
                    }
     ...
     * Delete an item
     * JP Updates
     * - Removed reference to google API
     * @param {int} index   Index of the item to be deleted
     ...
     * JP Updates
     * 1. - To enable opening external links for items that are events, set a "linkTo" (url) property 
     ...
     * Creates the DOM for the item, depending on its type
     * JP Updates
     * 1. - Added code to display the date inline below the title (to be displayed on hover/touch)
     * @return {Element | undefined}
     * @override
     */
    links.Timeline.ItemRange.prototype.createDOM = function (options) {
        // background box
        ...
        // contents box
        var divContent;
        if (this.className == "month-item") {
            divContent = document.createElement("BUTTON");
        }
        else { // it's a "month-event"
            if (this.linkTo) { // open link in new window  
                divContent = document.createElement("A");
                divContent.href = this.linkTo;
                divContent.target = "_blank";
            }
            else { // no link, so we don't need an anchor
                divContent = document.createElement("DIV");
            }
        }
        if (isInterface) {
            divContent.className = "timeline-event-content group";
            divContent.tabIndex = 0;
        }
        else {
            divContent.className = "timeline-event-content group unselectable";
        }
        //if (isInterface) {
        //// JP - set title attribute to interface event
        //divContent.setAttribute('title', this.content);
        //}
        //else {
        var divTitle = document.createElement("SPAN");
        divTitle.className = "content-title";
        divTitle.innerHTML = this.content;
        ...
        // 1.
        var itemData = {};
        itemData.start = new Date(this.start.valueOf());
        itemData.end = new Date(this.end.valueOf());

        var getStartMonth,
            getEndMonth,
            getStartDay;
        getStartMonth = options.MONTHS_SHORT[itemData.start.getMonth()];
        getEndMonth = options.MONTHS_SHORT[itemData.end.getMonth()];
        getStartDay = options.DAYS_SHORT[itemData.start.getDay()];

        var startFormatted = getStartMonth + " " +
                            itemData.start.getDate();/* + ", " +
                          itemData.start.getFullYear();*/
        var divDate = document.createElement("SPAN");
        divDate.className = "content-date";
        if (this.className == "month-item") {
            divDate.innerHTML = getStartMonth;
        }
        else {
            if (itemData.start.setHours(0, 0, 0, 0) < itemData.end.setHours(0, 0, 0, 0)) {
                var endFormatted = getEndMonth + " " +
                                  itemData.end.getDate();/* + ", " +
                            itemData.end.getFullYear();*/
                divDate.innerHTML = startFormatted;
                divDate.innerHTML += " &#8211; "; // en dash
                divDate.innerHTML += endFormatted;
            }
            else {
                var fullStartDay = getStartDay;
                divDate.innerHTML = fullStartDay + ", " + startFormatted;
            }
        }
     ...
     * @override
     */
    links.Timeline.ItemRange.prototype.showDOM = function (container, options) {
        var dom = this.dom;
        if (!dom) {
     ...
     * of the item
     * JP Updates
     * 1. - Commented out the resetting of the content to the innerHTML (don't need it)
     * @override
     */
    links.Timeline.ItemRange.prototype.updateDOM = function () {
        ...
        if (divBox) {
            // update contents
            //divBox.firstChild.innerHTML = this.content; // 1.
     ...
     * range of the timeline and the timeline options. 
     * JP Updates
     * 1. - Use titleWidth to set the width of the item (this will help with stacking
     * 2. - Add element to display the range of the event
     * 3. - Added 20 pixels to compensate for the fact that the proper width can't be captured 
     *      until the page has fully rendered (which hasn't happened at this point)
     * @param {links.Timeline} timeline
     * @override
     */
    links.Timeline.ItemRange.prototype.updatePosition = function (timeline) {
        ...
        if (dom) {
            ...
            /* DEBUG
            if (isScaffold && this.content == "International Education Week") {
                console.log(this.content + ' dom.style.width set to: ' + dom.style.width);
                //debugger;
            }*/
            ...
            if (titleWidth === undefined) {
                if ((' ' + dom.className + ' ').indexOf(' custom ') > -1) {
                    console.log('WARNING: The custom event range for "' + this.content + '"has an undefined titleWidth');
                }
            }
            else {
                var width = this.getWidth(timeline);
                var checkWidth = ((titleWidth - width) >= 1) ? (titleWidth + 0) : width; // 3. ( + 20)
                dom.style.width = checkWidth + "px";

                if (!dom.hasAttribute("data-title-width")) {
                    dom.setAttribute("data-title-width", checkWidth);
                }
                else if (titleWidth < width) {
                    dom.setAttribute("data-title-width", titleWidth);
                }

                // store range-width and title-width as data attributes
                dom.setAttribute("data-range-width", width);

                // set custom class on the range element
                links.Timeline.addClassName(dom, 'custom');

                // 2. - create "overlay" element to convey proper range size
                var overlayEl;
                if (dom.childNodes.length == 1) {
                    overlayEl = document.createElement("DIV");
                    overlayEl.className = "range-overlay";
                    overlayEl.style.width = width + "px";
                    dom.appendChild(overlayEl);
                }
                else if (dom.childNodes.length == 2) {
                    overlayEl = dom.getElementsByClassName("range-overlay")[0];
                    overlayEl.style.width = width + "px";
                }
            }
        }
     ...
     * JP Updates
     * 1. - Return formatted dates differently for single day events vs multi-day events
     * @param {Number} index
    ...
    links.Timeline.prototype.getItem = function (index) {
        ...
        // 1.
        var fullStartDay = this.options.DAYS[itemData.start.getDay()];
        var startFormatted = this.options.MONTHS_SHORT[itemData.start.getMonth()] + " " +
                              itemData.start.getDate() + ", " +
                              itemData.start.getFullYear();
        itemData.startFormatted = fullStartDay + " " + startFormatted;
        if (item.end) {
            itemData.end = new Date(item.end.valueOf());
            var endFormatted = this.options.MONTHS_SHORT[itemData.end.getMonth()] + " " +
                                itemData.end.getDate() + ", " +
                                itemData.end.getFullYear();
            itemData.startFormatted = startFormatted;
            itemData.endFormatted = endFormatted;
        }
        if (item.group) {
    ...
    links.Timeline.prototype.changeItem = function (index, itemData, preventRender) {
        ...
        var newItem = this.createItem({
            'nodeId': itemData.hasOwnProperty('nodeId') ? itemData.nodeId : oldItem.nodeId,
            'start': itemData.hasOwnProperty('start') ? itemData.start : oldItem.start,
            'end': itemData.hasOwnProperty('end') ? itemData.end : oldItem.end,
            'content': itemData.hasOwnProperty('content') ? itemData.title : oldItem.title,
            'description': itemData.hasOwnProperty('content') ? itemData.description : oldItem.description,
    ...
    /**
     * JP - Select an event (but don't move it to be centered)
     * For example selection = [{row: 5}];
     * @param {Array} selection   An array with a column row, containing the row
     *                           number (the id) of the event to be selected.
     * @return {boolean}         true if selection is succesfully set, else false.
     */
    links.Timeline.prototype.setSelectionInPlace = function (selection) {
        if (selection != undefined && selection.length > 0) {
            if (selection[0].row != undefined) {
                var index = selection[0].row;
                if (this.items[index]) {
            ...
            if (this.options.stackEvents) {
                do {
                    ...
                    if (collidingItem != null) {
                        ...
                        if (axisOnTop) {
                            // DEBUG console.log("interface collidingItem = " + collidingItem.item.content);
                        ...
                        if (isInterface) {
                          //debugger;
                          console.log("finalItem.height: " + finalItem.height)
                        }*/
    ...
    /**
     * JP Updates
     * 1. - Use "content" width for range if it's greater than the range's width
     */
    links.Timeline.prototype.initialItemsPosition = function (items, groupBase) {
        ...
        for (var i = 0, iMax = items.length; i < iMax; ++i) {
            ...
            // 1.
            var useContentWidth = false;
            var contentWidth;
            if (items[i].contentWidth != undefined) {
                if (item.contentWidth > width) {
                    contentWidth = item.contentWidth;
                    useContentWidth = true;
                }
            }
            var right = (useContentWidth) ? item.getRight(this) - (contentWidth - item.width)
                                          : item.getRight(this);
            var left = right - width;

            /* DEBUG
            if (isScaffold && item.content == "International Education Week") {
                //console.log(item.content + ' dom.style.width set to: ' + dom.style.width);
                debugger;
            }  */
     ...
     * JP Updates
     * 1. - Set the proper stack order for the scaffold to be used by the interface by
     *      preventing items from colliding based on titleWidth
     * 2. - If the event is a "month" (added for user to sort the cards), then don't 
     *      include in the stacking logic, exit the method
    ...
    links.Timeline.prototype.collision = function (item1, item2, margin) {
        ...
        // 2.
        if (item1.item.dom.className.indexOf("month-item") > -1) {
            return;
        }
        // 1.
        if (isScaffold) {
            var tWidth1 = parseFloat(item1.item.dom.getAttribute("data-title-width"));
            if (isNaN(tWidth1)) {
                tWidth1 = item1.item.dom.clientWidth;
            }
            else {
                var cWidth = item1.item.dom.clientWidth;
                tWidth1 = (tWidth1 > cWidth) ? tWidth1 : cWidth;
            }

            var tWidth2 = parseFloat(item2.item.dom.getAttribute("data-title-width"));
            if (isNaN(tWidth2)) {
                tWidth2 = item1.item.dom.clientWidth;
            }
            else {
                var cWidth = item1.item.dom.clientWidth;
                tWidth2 = (tWidth2 > cWidth) ? tWidth2 : cWidth;
            }

            /* DEBUG
            if (isScaffold) {
              if (item1.item.content == "Transcripts - July") {
                console.log("item1.item.content = " + item1.item.content);
                console.log("item2.item.content = " + item2.item.content);
                console.log("Check Return statement:");
                //var chk1 = item1.left - (tWidth1 - (item1.right - item1.left)) < item2.right + (tWidth2 - (item2.right - item2.left));
                var chk1 = item1.left - margin < item2.right + (tWidth2 - (item2.right - item2.left));
                console.log("check 1 = " + chk1);
                var chk2 = item1.right + (tWidth1 - (item1.right - item1.left)) + margin > item2.left;
                console.log("check 2 = " + chk2);
                //var eq = item1.right + (tWidth1 - (item1.right - item1.left));
                //console.log("check 2: item1.right + (tWidth1 - (item1.right - item1.left)) = " + eq);
                //console.log("check 2: item2.left = " + item2.left);
                var chk3 = item1.top - margin < item2.bottom;
                console.log("check 3 = " + chk3);
                var chk4 = item1.bottom + margin > item2.top;
                console.log("check 4 = " + chk4);
                var final = (item1.left - margin < item2.right + (tWidth2 - (item2.right - item2.left)) &&
                  item1.right + (tWidth1 - (item1.right - item1.left)) + margin > item2.left &&
                  item1.top - margin < item2.bottom &&
                  item1.bottom + margin > item2.top);
                console.log("ACTUAL return = " + final);
                //debugger;
              }
            } */

            return (item1.left - margin < item2.right + (tWidth2 - (item2.right - item2.left)) &&
                item1.right + (tWidth1 - (item1.right - item1.left)) + margin > item2.left &&
                item1.top - margin < item2.bottom &&
                item1.bottom + margin > item2.top);
     ...
     * formatted as "hh:mm".
     * JP Updates
     * - Added conditional to display different date format for scaffold vs interface, but
     *   don't think this is needed (so commented it out)
     * @param {Object} options
     ...
     * JP Updates
     * 1. - Changed SCALE.DAY to return MONTH_SHORT instead of full MONTH
     * @param {Object} options
     * @param {Date} [date] custom date. if not provided, current date is taken
     */
